...
use crate::models::{MerchantPayReq, MerchantRegisterReq, SettlementReceipt};
...
pub async fn merchant_pay(
    ...
    let (tx_id, user_id) = {
        ...
        let (user_id, user_balance) = match existing {
            Ok((id, bal)) => (id, bal),
            Err(_) => {
                return HttpResponse::NotFound()
                    .json(serde_json::json!({"error": "Payer account not found"}))
            }
        };
...
pub async fn register_merchant(
    ...
    if exists > 0 {
        ...
        let expected_registration_token = match env::var("MERCHANT_REGISTRATION_TOKEN") {
            Ok(token) if !token.is_empty() => token,
            _ => {
                return HttpResponse::Forbidden()
                    .json(serde_json::json!({"error": "Merchant self-registration disabled"}))
            }
        };
        if body.registration_token.as_deref() != Some(expected_registration_token.as_str()) {
            return HttpResponse::Forbidden()
